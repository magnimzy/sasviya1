...
%let ratio=50; /* unlabelled to true positive ratio */
...
%macro logreg(data=mycas.part, target=target, IntIn=V1-V28 Amount, metric=precision, iter=1);
    ...
    ods output ParameterEstimates=Coeff_&iter.(keep=effect estimate rename= (estimate=estimate_&iter.));
	...
	proc sort data=Coeff_1; by effect;	run;

    proc sort data=Coeff_&iter.; by effect;	run;

	data Coeff_1;		/* Store logreg coefficients estimates*/
		merge Coeff_1 Coeff_&iter.;
        by effect;
	run;
...
%macro ensemble_creation(ul_tp=&ratio, metrics=mycas.eval_metric);
	...
	%do i=1 %to &N_metrics;
		...
		%do j=1 %to &final_num_models.;
            %bootstrap(data=mycas.training, target=target, ul_tp=&ul_tp., i=&j.);/* create sample mycas.part that will
...
ensemble=svm_ensemble, metric=f1, ul_tp=&ratio.);
    ...
    data mycas.&metric._svm_eval;
        ...
        *if p&target.1_svm_Avg>=0.5 then svm_soft_vote = '1'; else svm_hard_vote = '0';
        if i&target._svm_Avg>=0.5 then svm_hard_vote = '1'; else svm_soft_vote = '0';
        keep ID &target svm_hard_vote svm_soft_vote;
	run;
...
ensemble=logreg_ensemble, metric=f1, ul_tp=&ratio.);
    ...
    data mycas.&metric._logreg_eval;
        ...
        if p&target.1_logreg_Avg>=0.5 then logreg_soft_vote = '1'; else logreg_hard_vote = '0';
        if i&target._logreg_Avg>=0.5 then logreg_hard_vote = '1'; else logreg_soft_vote = '0';
        keep ID &target logreg_hard_vote logreg_soft_vote;
	run;
     
    /* Mean coefficient */
	data mycas.Coeff(keep=effect mean_coef); /* Store logreg coefficients estimates*/
		set Coeff_1; mean_coef=mean(of estimate_1-estimate_&N.);
	run;
	...
	proc sql;
		select effect into :my_column_order separated by " " from mycas.coeff order by effect;
	quit;
	...
	proc iml;
		use mycas.coeff; read all var {mean_coef} into x; close;
	...
	data mycas.pred;
		...
		set pred1;pred=logistic(col1);
        if pred>=0.5 then mean_coef_vote = '1'; mean_coef_vote = '0';
	run;

   %metric_compute(data=mycas.pred, target=target, pred_target=pred); /** Gives mycas.metrics that 
...
ensemble=gboost_ensemble, metric=f1, ul_tp=&ratio.);
    ...
    data mycas.&metric._gboost_eval;
        ...
        if p&target.1_gboost_Avg>=0.5 then gboost_soft_vote = '1'; else gboost_hard_vote = '0';
        if i&target._gboost_Avg>=0.5 then gboost_hard_vote = '1'; else gboost_soft_vote = '0';
...
proc export data=mycas.evaluations
    outfile="&outdir.evaluations.csv"
    dbms=csv;
run;
