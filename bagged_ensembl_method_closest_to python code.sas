...
ensemble=svm_ensemble, metric=f1, ul_tp=&ratio.);
    ...
    data mycas.&metric._svm_eval1;
        ...
        /*if p&target.1_svm_Avg>=&cutoff. then svm_soft_vote = '1'; else svm_hard_vote = '0';*/
        if i&target._svm_Avg>=&cutoff. then svm_hard_vote = '1'; else svm_hard_vote = '0';
...
ensemble=logreg_ensemble, metric=f1, ul_tp=&ratio.);
    ...
    data mycas.&metric._logreg_eval1;
        ...
        if p&target.1_logreg_Avg>=&cutoff. then logreg_soft_vote = '1'; else logreg_soft_vote = '0';
        if i&target._logreg_Avg>=&cutoff. then logreg_hard_vote = '1'; else logreg_hard_vote = '0';
        keep ID &target logreg_hard_vote logreg_soft_vote p&target.1_logreg_Avg i&target._logreg_Avg;
	run;
    ...
    %do j=0 %to 30;
	   ...
	   %metric_compute_prob(data=mycas.pred, target=target, pred_prob=pred, prob=%sysevalf(&pb)); /** Gives 
...
ensemble=gboost_ensemble, metric=f1, ul_tp=&ratio.);
    ...
    data mycas.&metric._gboost_eval1;
        ...
        if p&target.1_gboost_Avg>=&cutoff. then gboost_soft_vote = '1'; else gboost_soft_vote = '0';
        if i&target._gboost_Avg>=&cutoff. then gboost_hard_vote = '1'; else gboost_hard_vote = '0';
        keep ID &target gboost_hard_vote gboost_soft_vote p&target.1_gboost_Avg i&target._gboost_Avg;
	run;
...
proc export data=mycas.evaluations
    outfile="&outdir.evaluations11.csv"
    dbms=csv replace;
run;
