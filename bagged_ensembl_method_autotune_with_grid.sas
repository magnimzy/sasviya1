...
%macro metric_compute_prob(data=, target=, pred_prob=, prob=);
	data assess1;
		set &data;
        pred_target=(&pred_prob.>=&prob.);
		if &target='1' and pred_target='1' then    tp=1; else  tp=0;
		if &target='1' and pred_target='0' then    fn=1; else	fn=0;
		if &target='0' and pred_target='1' then	   fp=1; else	fp=0;
		if &target='0' and pred_target='0' then	   tn=1; else	tn=0;
...
%macro svm(data=mycas.part, target=target, IntIn=V1-V28 Amount, metric=f1, iter=1);
		...
		proc svmachine data=&data.;
			...
			autotune searchmethod=grid objective=&metric. tuningparameters=(c(values=000.01 000.10 001.00 010.00 100.00) degree(values=1 2 3));
		...
		proc astore;
			score data=&valid. out=mycas.svm_pred_&metric._&iter.
...
%macro logreg(data=part, target=target, IntIn=V1-V28 Amount, metric=f1, iter=1);
		...
		proc cas noqueue;
			autotune.tuneLogistic / 
                scoreoptions={table={name="&data.", caslib="casuser", where="_partind_=0"}}, 
                trainOptions={table={name="&data.", caslib="casuser", where="_partind_=1"}, 
				model={depVars={{name="&target."}}, 
				effects={{vars=${&Intin.}}}}, savestate={name="logreg_&metric._&iter."} } 
				useParameters="CUSTOM",
                tunerOptions={seed=12345, objective="&metric.", userdefinedpartition=TRUE, searchmethod="GRID"} 
				tuningParameters={{name='LASSORHO', valuelist={0.01 0.10 0.20 0.30 0.40 0.50 0.60 0.70 0.80 0.90 0.99}}};
		...
		proc astore;
			score data=&valid. out=mycas.logreg_pred_&metric._&iter.
...
%macro gboost(data=mycas.part, target=target, IntIn=V1-V28 Amount, metric=f1, iter=1);
		...
		proc gradboost data=&data. earlystop(stagnation=0);
			partition rolevar=_partind_(train='1' validate='0');
			autotune searchmethod=grid 
				objective=&metric. tuningparameters=(ntrees(values=100 250 500) 
				maxdepth(values=1 2 3 4) learningrate(values= 0.10 0.25 0.50 1.00) lasso(EXCLUDE) 
		...
		proc astore;
			score data=&valid. out=mycas.gboost_pred_&metric._&iter.
        ...
        data mycas.out;
            ...
            keep ID p_&target.1 p_&target.0 i_&target. &target.;
			rename p_&target.0 = p&target.0_gboost_&iter. p_&target.1=p&target.1_gboost_&iter. i_&target.=i&target._gboost_&iter.;
		run;
...
/* Saving model ensembles features */
proc export data=gboost_ensemble
    outfile="&outdir.gboost_ensemble.csv"
    dbms=csv replace;
run;

proc export data=logreg_ensemble
    outfile="&outdir.logreg_ensemble.csv"
    dbms=csv replace;
run;

proc export data=svm_ensemble
    outfile="&outdir.svm_ensemble.csv"
    dbms=csv replace;
run;
...
%macro logreg_eval(metric=misc, ul_tp=&ratio., target=target);
	...
	data mycas.&metric._logreg_eval1;
        ...
        keep ID &target i&target._logreg_Avg logreg_hard_vote p&target.1_logreg_Avg;
	run;
   ...
   %do j=0 %to 30;
       ...
       %let pb=%sysevalf(0.25+0.025*&j.);

	   %metric_compute_prob(data=mycas.&metric._logreg_eval1, target=target, pred_prob=p&target.1_logreg_Avg, prob=%sysevalf(&pb)); /** Gives 
	   mycas.metrics that contain the performance evaluation for this ensemble for soft_vote***/
	
		data mycas.results;
			set mycas.metrics;
			ensemble="logreg_soft_vote_&metric._ul_&ul_tp._prob_&pb.";
		run;
	...
	Data B(keep=Intercept Amount V1-V28); set &valid.; intercept=1; /* Get the covariates */
	run;
	...
	%do j=0 %to 30;
		%let pb=%sysevalf(0.25+0.025*&j.);
		%metric_compute_prob(data=mycas.pred, target=target, pred_prob=pred, prob=%sysevalf(&pb));

		data mycas.results;
			set mycas.metrics;
			ensemble="logreg_mean_coef_&metric._ul_&ul_tp._prob_&pb.";
		run;
...
%macro gboost_eval(metric=misc, ul_tp=&ratio.,target=target);
   ...
   %do j=0 %to 30;
		%let pb=%sysevalf(0.25+0.025*&j.);
		%metric_compute_prob(data=mycas.&metric._gboost_eval1, target=target, pred_prob=p&target.1_gboost_Avg, prob=%sysevalf(&pb));

		data mycas.results;
			set mycas.metrics;
			ensemble="gboost_soft_vote_&metric._ul_&ul_tp._prob_&pb.";
		run;
